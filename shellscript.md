...
```
もちろん上の例なら、よりシンプルに`seq`で対応可能。ただし、**`seq`コマンドはPOSIXではない！**
```shell
...
echo "$i"
```
上記の様に、`set`コマンドを明示的に仕様してもよいが、シバンにオプションとして組み込むこともできる。
```shell
#!/bin/sh -u
echo "$i"
```
...
declare|grep ushinosuke
```
ちなみに、**ここで登場する`declare`コマンドは`bash`独自の組み込みコマンドである。**POSIXコンプライアントで変数定義の確認をするには、Recipe2.0を使えば良い。
```shell
${ushinosuke+:} false
echo $?
```
...
awk 'BEGIN{print $x}'
```
**参考までに**「高度」な計算ではないが`bc`コマンドを使う手もある。`PIPESTATUS`（拡張機能）との併用で、エラーチェックができる。
```shell
...
```shell
string="文字列"
echo $string | awk '$0=length($0)'
```
ただし`AWK`（`GNU Awk`ではない！）はASCIIにしか対応しない（？）ので、日本語の場合はバイト数を返す。ゆえに`AWK`は日本語との併用は控えた方が良い。
...
以下は最後から2番目のフィールドの値（d）を得る方法。勿論`AWK`を使う。シェルスクリプトと`AWK`を組み合わせる時には、`AWK`のパターンをシングルクォート`'`ではなくダブルクォート`"`で表すこと、そして`$`の前にバックスラッシュ`¥`を「2つ」つけることがポイント。**以下の例では「禁じ手の」ブレース展開を使っているが、目を瞑ってほしい。**
```shell
...
単純な操作なら`cut`コマンドで十分。フィールド区切り文字を`-d`オプション、抽出するフィールドを`-f`オプションで指定する。プラスαが欲しい時は`AWK`を発動する。

※ `AWK`は確かに便利だが、簡単な操作であれば`cut`の方が簡単でタイプ量も少なく済む。

#### Recipe5.8 1列ごとに処理する
1行ごとにデータを抽出した後に、それを`for`文や`AWK`で切り分ければよい。以下、3つの処方箋を紹介する。
1. その1 - 単純に`for`文で分ける
```shell
# 変数lineに1行分のデータが入っているものとする
for filed in $line; do
    # 処理
done
```
このレシピが最も単純で高速。しかし、前述の通り`for`文はデフォルトのフィールド区切り文字が「空白・タブ・改行」である。line内に「空白・タブ」が含まれていると、このレシピは使えない。データの仕様を自分で決められるのであれば、このレシピを適用できるフォーマット（例えばCSV）にすればよいだろう。
2. その2 - エスケープしてから`for`文で切り分ける
```shell
# 変数lineに1行分のデータが入っているものとする
FS=','
for field in `echo "$line" | tr "$FS ¥¥¥¥t" ' ¥006¥025'`; do
    field=`echo "$line" | tr '¥006¥025' ' ¥t'`
    # 処理
done
```
この方法が一般的か。環境変数`IFS`を改変する方法と併用できないことに注意。
3. その3 - `AWK`で切り分ける
```shell
FS=','
echo "$line"                                                      ¥
| awk -F "$FS" '{n=split($0, f); for(i=1;i<=n;i++){print f[i];}}' ¥
| while read field; do
    # 処理
done
```
これは特殊用途。コントロールコード（ASCIIコードの0番から31番）を含むため、上記「その2」の方法が使えない時に用いる。ただし、`while`文にパイプを通してデータを渡しているため、`while`節の中から変数を取り出せないことに注意。

---

### ファイル操作

#### Recipe6.2 タイムスタンプを変更する
`touch`コマンドを使う。以下では、/home/cooking/diary.txtのタイムスタンプを2005年8月14日17時10分20秒に変更している。

```shell
[ -f /home/cooking/diary.txt ] && touch -t 20050814171020 /home/cooking/diary.txt
```
`touch`は0バイトファイルを作るのに使われることが多いが、本来の機能はタイムスタンプの修正である。

#### Recipe6.3 指定ディレクトリ以下のファイルの総サイズを知る
